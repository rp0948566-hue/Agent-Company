...
if grep -q 'MC_\${safe_p}_A_\${safe_a}_P_\${safe_ph}_R_\${safe_r}' "$_ORCH_ALL_TMP"; then
...
if grep -q "CACHE_\${provider}_\${agent_type}" "$_ORCH_ALL_TMP"; then
...
if grep -q 'config_file.*-nt.*persistent_cache' "$_ORCH_ALL_TMP"; then
...
if grep -q 'shell metacharacters\|spaces.*quotes' "$_ORCH_ALL_TMP"; then
...
if grep -q 'OCTOPUS_TRACE_MODELS' "$_ORCH_ALL_TMP"; then
...
if grep -q '\[model-trace\] Resolving:' "$_ORCH_ALL_TMP"; then
...
trace_tiers=$(grep -c '\[model-trace\] Tier' "$_ORCH_ALL_TMP" 2>/dev/null || echo "0")
...
if grep -q '\[model-trace\] ► Result:' "$_ORCH_ALL_TMP"; then
...
if grep -q '\[model-trace\].*>&2' "$_ORCH_ALL_TMP"; then
...
set_calls=$(grep -c 'atomic_json_update.*config_file' "$_ORCH_ALL_TMP" 2>/dev/null || echo "0")
...
if grep -q 'persistent_cache.*\.tmp\.\$\$' "$_ORCH_ALL_TMP"; then
...
if grep -q "jq --arg key.*--arg val.*persistent_cache" "$_ORCH_ALL_TMP"; then
...
if grep -q '^display_session_metrics()' "$_ORCH_ALL_TMP"; then
...
if grep -q '^display_provider_breakdown()' "$_ORCH_ALL_TMP"; then
...
if grep -q '^display_per_phase_cost_table()' "$_ORCH_ALL_TMP"; then
...
if grep -q '^record_agent_start()' "$_ORCH_ALL_TMP"; then
...
if grep -q '^record_agent_complete()' "$_ORCH_ALL_TMP"; then
...
if grep -q 'display_session_metrics' "$_ORCH_ALL_TMP" && \
   grep -q 'display_provider_breakdown' "$_ORCH_ALL_TMP" && \
   grep -q 'display_per_phase_cost_table' "$_ORCH_ALL_TMP"; then
...
if grep -q '^check_provider_health()' "$_ORCH_ALL_TMP"; then
...
if grep -q '^check_all_providers()' "$_ORCH_ALL_TMP"; then
...
if grep -q '^find_capable_fallback()' "$_ORCH_ALL_TMP"; then
...
if grep -q '^detect_project_quality_commands()' "$_ORCH_ALL_TMP"; then
...
if grep -q '^run_project_quality_checks()' "$_ORCH_ALL_TMP"; then
...
if grep -q '^cleanup_old_results()' "$_ORCH_ALL_TMP"; then
...
if grep -q 'OCTOPUS_COMPACT_BANNERS' "$_ORCH_ALL_TMP"; then
...
if grep -q '^format_workflow_banner()' "$_ORCH_ALL_TMP"; then
...
if grep -q '^score_result_file()' "$_ORCH_ALL_TMP"; then
...
if grep -q '^rank_results_by_signals()' "$_ORCH_ALL_TMP"; then
